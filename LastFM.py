...
class LastFM:
    ...
    def load_data(self) -> None:
        name = ("http://ws.audioscrobbler.com/2.0/" +
                "?method=user.getlovedtracks&user=")
        url = str(name + self.user + "&api_key=" + self.apikey + "&limit=" +
                  self.limit + "&page=" + self.page + "&format=json")

        r_obj = LastFM._download_url(self, url)
        ...
        if r_obj.get("error"):
            ...
            for track in r_obj["lovedtracks"]["track"]:
                ...
                track_date = datetime.fromtimestamp(date).strftime('%m/%d/%Y')

                artist_list.append(track_artist)
                name_list.append(track_name)
                date_list.append(track_date)
                track_list.append("Artist: " + track_artist + ", Track: " +
                                  track_name + ", Loved on: " + track_date)
